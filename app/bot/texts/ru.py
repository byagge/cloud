...
TEXTS = {
    ...
    "srv_btn_deploy": "📦 Обновить проект (zip)",
    "srv_btn_ai": "🤖 ИИ: правки проекта (Beta)",
    ...
    "agent_need_password": "Пароль SSH ещё не готов. Откройте сервер позже или напишите в поддержку.",
    "reinstall_os_groups": "<b>Смена ОС</b>\n\nВыберите группу:",
    "reinstall_os_versions": "<b>Смена ОС: {group}</b>\n\nВыберите образ:",
    "agent_deploy_ask": (
        "📦 <b>Обновление существующего проекта</b>\n\n"
        "Пришлите <b>zip/tar</b> для проекта, который <b>уже есть</b> на VPS.\n"
        "Затем выберите: перезаписать или рядом. ИИ только обновляет /opt/arix-apps/…\n"
        "В подписи можно указать, какой app обновить.\n\n"
        "Во время работы будет кнопка <b>Стоп</b>."
    ),
    "agent_ai_ask": (
        "🤖 <b>ИИ по проекту</b>\n\n"
        "Опишите, что сломалось (можно фото)."
    ),
    "agent_deploy_strategy_ask": (
        "📦 Архив сохранён.\n\n"
        "Как деплоить на этом сервере?"
    ),
    "agent_btn_dep_over": "♻️ Перезаписать существующий",
    "agent_btn_dep_side": "➕ Развернуть рядом",
    "agent_btn_plan_go": "✅ Работать по плану",
    "agent_btn_plan_no": "❌ Отклонить план",
    "agent_plan_accepted": "⚙️ Начинаю работу по плану…",
    "agent_answer": "✅ {answer}\n\n💡 {suggestion}",
    "agent_plan_use_buttons": "Используйте кнопки под планом (Работать по плану / Стоп).",
    "agent_need_gemini": "Не задан ни один LLM ключ (GEMINI / OPENAI / ANTHROPIC) в .env.",
    "agent_oops": "Что-то пошло не так, мы уже знаем об этом и скоро все исправим.",
    "agent_beta_terms": (
        "⚠️ <b>ИИ-помощник (Beta)</b>\n\n"
        "Это экспериментальная функция с ограниченным использованием.\n\n"
        "• ИИ может ошибаться, предлагать неверные шаги или неполный анализ\n"
        "• Всегда проверяйте изменения на сервере перед продакшеном\n"
        "• Не передавайте секреты/пароли в чат, если это не необходимо\n"
        "• Недельный лимит токенов; обновление — каждый <b>Понедельник 00:00</b>\n\n"
        "Продолжая, вы понимаете риски Beta и ограничения использования."
    ),
    "btn_ai_beta_accept": "Понятно, продолжить",
    "agent_budget": "Лимит исчерпан, обновление лимита в {reset}.",
    "agent_budget_reset": "Понедельник 00:00",
    "agent_started": (
        "🛠 <b>Сессия ИИ запущена</b>\n\n"
        "• Только существующие проекты\n"
        "• Спросит, если нужен токен/домен\n"
        "• <b>Стоп</b> — отмена в любой момент"
    ),
    "agent_got_answer": "Принято — ИИ продолжает. Стоп всё ещё доступен.",
    "agent_ask_user": "❓ ИИ спрашивает:\n\n{question}\n\nОтветьте одним сообщением (или Стоп).",
    "agent_done": "✅ {summary}",
    "agent_fail": "❌ Агент: {err}",
    "agent_busy": "ИИ ждёт ваш ответ на предыдущий вопрос.",
    "agent_no_ip": "У сервера ещё нет IP.",
    "agent_file_too_big": "Файл слишком большой (макс. 40 МБ).",
    "agent_file_bad_type": "Пришлите архив zip / tar / tar.gz.",
    "agent_need_detail": "Опишите проблему подробнее (минимум 3 символа).",
    "agent_nopw_reset": "Нет пароля SSH — сбрасываем. Запустите ИИ снова через минуту.",
    "agent_already_running": "ИИ уже работает на этом сервере. Дождитесь завершения.",
    "agent_locked": "ИИ занят на этом сервере. Повтор через минуту…",
    "agent_btn_stop": "⏹ Стоп ИИ",
